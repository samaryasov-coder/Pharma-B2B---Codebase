<?php /* Smarty version Smarty-3.1.14, created on 2026-08-24 17:42:35
         compiled from "wa-apps/site/templates/actions/themes/Design.html" */ ?>
<?php /*%%SmartyHeaderCode:8113600736a8c828b9b6dd8-36679545%%*/if(!defined('SMARTY_DIR')) exit('no direct access allowed');
$_valid = $_smarty_tpl->decodeProperties(array (
  'file_dependency' => 
  array (
    '64d48f26b4047e5c91b3e8efd29d24e83e06c2a6' => 
    array (
      0 => 'wa-apps/site/templates/actions/themes/Design.html',
      1 => 1779891735,
      2 => 'file',
    ),
  ),
  'nocache_hash' => '8113600736a8c828b9b6dd8-36679545',
  'function' => 
  array (
    'skeleton_sidebar' => 
    array (
      'parameter' => 
      array (
      ),
      'compiled' => '',
    ),
    'renderFileTree' => 
    array (
      'parameter' => 
      array (
        'theme_files' => 
        array (
          'blockpage.footer.html' => 
          array (
            'custom' => false,
            'parent' => false,
            'description' => 'Контент для блока Шапка внутри блочных страниц',
            'type' => 'file',
            'path' => 'blockpage.footer.html',
          ),
          'blockpage.header.html' => 
          array (
            'custom' => false,
            'parent' => false,
            'description' => 'Контент для блока Шапка внутри блочных страниц',
            'type' => 'file',
            'path' => 'blockpage.header.html',
          ),
          'blockpage.layout.html' => 
          array (
            'custom' => false,
            'parent' => false,
            'description' => 'Лэйаут для блочных страниц',
            'type' => 'file',
            'path' => 'blockpage.layout.html',
          ),
          'blockpage.wrapper.html' => 
          array (
            'custom' => false,
            'parent' => false,
            'description' => 'Оформление блочных страниц',
            'type' => 'file',
            'path' => 'blockpage.wrapper.html',
          ),
          'default.css' => 
          array (
            'custom' => false,
            'parent' => false,
            'description' => 'Основной файл CSS-стилей семейства тем дизайна «Дефолт 5.0»',
            'type' => 'file',
            'path' => 'default.css',
          ),
          'default.js' => 
          array (
            'custom' => false,
            'parent' => false,
            'description' => '',
            'type' => 'file',
            'path' => 'default.js',
          ),
          'error.html' => 
          array (
            'custom' => false,
            'parent' => false,
            'description' => '404 и другие ошибки',
            'type' => 'file',
            'path' => 'error.html',
          ),
          'footer.html' => 
          array (
            'custom' => false,
            'parent' => false,
            'description' => 'Нижняя часть футера приложения с информациекй о разработчике',
            'type' => 'file',
            'path' => 'footer.html',
          ),
          'forgotpassword.html' => 
          array (
            'custom' => false,
            'parent' => false,
            'description' => 'Страница сброса пароля',
            'type' => 'file',
            'path' => 'forgotpassword.html',
          ),
          'head.html' => 
          array (
            'custom' => false,
            'parent' => false,
            'description' => 'Блок <head> приложения «Сайт»',
            'type' => 'file',
            'path' => 'head.html',
          ),
          'header.html' => 
          array (
            'custom' => false,
            'parent' => false,
            'description' => 'Меню страниц приложения',
            'type' => 'file',
            'path' => 'header.html',
          ),
          'header.section.html' => 
          array (
            'custom' => false,
            'parent' => false,
            'description' => 'Содержимое «шапки» приложения',
            'type' => 'file',
            'path' => 'header.section.html',
          ),
          'index.html' => 
          array (
            'custom' => false,
            'parent' => false,
            'description' => 'Основной макет тем дизайна семейства «Дефолт 5.0»',
            'type' => 'file',
            'path' => 'index.html',
          ),
          'login.html' => 
          array (
            'custom' => false,
            'parent' => false,
            'description' => 'Страница входа для пользователей',
            'type' => 'file',
            'path' => 'login.html',
          ),
          'main.html' => 
          array (
            'custom' => false,
            'parent' => false,
            'description' => 'Основной вывод приложения «Сайт»',
            'type' => 'file',
            'path' => 'main.html',
          ),
          'my.nav.html' => 
          array (
            'custom' => false,
            'parent' => false,
            'description' => 'Навигационное меню внутри личного кабинета',
            'type' => 'file',
            'path' => 'my.nav.html',
          ),
          'my.profile.html' => 
          array (
            'custom' => false,
            'parent' => false,
            'description' => 'Личный кабинет: редактирование контактной информации',
            'type' => 'file',
            'path' => 'my.profile.html',
          ),
          'page.html' => 
          array (
            'custom' => false,
            'parent' => false,
            'description' => 'Содержимое открытой страницы и навигация по страницам',
            'type' => 'file',
            'path' => 'page.html',
          ),
          'signup.html' => 
          array (
            'custom' => false,
            'parent' => false,
            'description' => 'Страница регистрации пользователей',
            'type' => 'file',
            'path' => 'signup.html',
          ),
        ),
        'level' => 0,
        'parent_path' => '',
      ),
      'compiled' => '',
    ),
    'skeleton' => 
    array (
      'parameter' => 
      array (
      ),
      'compiled' => '',
    ),
  ),
  'variables' => 
  array (
    'wa' => 0,
    'options' => 0,
    'wa_url' => 0,
    'app' => 0,
    'route' => 0,
    'theme' => 0,
    '_route_label' => 0,
    'themes_routes' => 0,
    'r' => 0,
    'themes' => 0,
    '_theme' => 0,
    'design_url' => 0,
    '_r_label' => 0,
    '_theme_is_trial' => 0,
    'current_url' => 0,
    '_url' => 0,
    '_title' => 0,
    'themes_url' => 0,
    'edit_data' => 0,
    'level' => 0,
    'theme_files' => 0,
    'f' => 0,
    'parent_path' => 0,
    'f_id' => 0,
    'current_dir_path' => 0,
    '_level_padding' => 0,
    'is_expanded' => 0,
    'module_name' => 0,
  ),
  'has_nocache_code' => 0,
  'version' => 'Smarty-3.1.14',
  'unifunc' => 'content_6a8c828ba078d0_78489139',
),false); /*/%%SmartyHeaderCode%%*/?>
<?php if ($_valid && !is_callable('content_6a8c828ba078d0_78489139')) {function content_6a8c828ba078d0_78489139($_smarty_tpl) {?><?php if (!is_callable('smarty_modifier_truncate')) include '/var/www/pharmab2b/httpdocs/wa-system/vendors/smarty3/plugins/modifier.truncate.php';
?><div class="s-design content flexbox">
    <?php if (!$_smarty_tpl->tpl_vars['wa']->value->isMobile()){?><div class="content flexbox space-16"><?php }?>
        <?php if (!function_exists('smarty_template_function_skeleton_sidebar')) {
    function smarty_template_function_skeleton_sidebar($_smarty_tpl,$params) {
    $saved_tpl_vars = $_smarty_tpl->tpl_vars;
    foreach ($_smarty_tpl->smarty->template_functions['skeleton_sidebar']['parameter'] as $key => $value) {$_smarty_tpl->tpl_vars[$key] = new Smarty_variable($value);};
    foreach ($params as $key => $value) {$_smarty_tpl->tpl_vars[$key] = new Smarty_variable($value);}?>
            <div class="skeleton box flexbox vertical">
                <span class="skeleton-line"></span>
                <span class="skeleton-header"></span>
                <?php $_smarty_tpl->tpl_vars['i'] = new Smarty_Variable;$_smarty_tpl->tpl_vars['i']->step = 1;$_smarty_tpl->tpl_vars['i']->total = (int)ceil(($_smarty_tpl->tpl_vars['i']->step > 0 ? 3+1 - (1) : 1-(3)+1)/abs($_smarty_tpl->tpl_vars['i']->step));
if ($_smarty_tpl->tpl_vars['i']->total > 0){
for ($_smarty_tpl->tpl_vars['i']->value = 1, $_smarty_tpl->tpl_vars['i']->iteration = 1;$_smarty_tpl->tpl_vars['i']->iteration <= $_smarty_tpl->tpl_vars['i']->total;$_smarty_tpl->tpl_vars['i']->value += $_smarty_tpl->tpl_vars['i']->step, $_smarty_tpl->tpl_vars['i']->iteration++){
$_smarty_tpl->tpl_vars['i']->first = $_smarty_tpl->tpl_vars['i']->iteration == 1;$_smarty_tpl->tpl_vars['i']->last = $_smarty_tpl->tpl_vars['i']->iteration == $_smarty_tpl->tpl_vars['i']->total;?>
                    <span class="skeleton-list"></span>
                <?php }} ?>
            </div>
        <?php $_smarty_tpl->tpl_vars = $saved_tpl_vars;
foreach (Smarty::$global_tpl_vars as $key => $value) if(!isset($_smarty_tpl->tpl_vars[$key])) $_smarty_tpl->tpl_vars[$key] = $value;}}?>


        <?php if ($_smarty_tpl->tpl_vars['options']->value['js']['ace']){?><script src="<?php echo $_smarty_tpl->tpl_vars['wa_url']->value;?>
wa-content/js/ace/ace.js"></script><?php }?>
        <?php if ($_smarty_tpl->tpl_vars['options']->value['js']['editor']){?><script src="<?php echo $_smarty_tpl->tpl_vars['wa_url']->value;?>
wa-content/js/jquery-wa/wa.elrte.ace.js?v<?php echo $_smarty_tpl->tpl_vars['wa']->value->version();?>
"></script><?php }?>
        <link rel="stylesheet" href="<?php echo $_smarty_tpl->tpl_vars['wa_url']->value;?>
wa-content/css/wa/design.css?<?php echo $_smarty_tpl->tpl_vars['wa']->value->version();?>
">

        <div class="sidebar blank flexbox width-16rem<?php if ($_smarty_tpl->tpl_vars['wa']->value->isMobile()){?> height-auto<?php }?>"
            <?php if ($_smarty_tpl->tpl_vars['wa']->value->isMobile()){?>style="position: relative; width:100%; min-height: auto; top: 0"<?php }else{ ?>style="height: calc(100vh - var(--sidebar-scroll-offset, 4rem));"<?php }?>
        >
            <div class="sidebar-header box custom-py-0">

                
                <div class="blank box rounded dropdown flexbox middle js-theme-select-menu wa-theme-select-menu hidden">
                    <img src="<?php echo $_smarty_tpl->tpl_vars['wa_url']->value;?>
<?php echo $_smarty_tpl->tpl_vars['app']->value['icon'][48];?>
" width="48" class="custom-mr-12">
                    <a href="#" class="dropdown-toggle wide" id="wa-theme-active">
                        <span class="js-url bold">
                            <?php if ($_smarty_tpl->tpl_vars['route']->value){?>
                                <?php $_smarty_tpl->tpl_vars['_route_label'] = new Smarty_variable(($_smarty_tpl->tpl_vars['route']->value['_domain']).('/').($_smarty_tpl->tpl_vars['route']->value['url']), null, 0);?>
                            <?php }else{ ?>
                                <?php $_smarty_tpl->tpl_vars['_route_label'] = new Smarty_variable($_smarty_tpl->tpl_vars['theme']->value['name'], null, 0);?>
                            <?php }?>
                            <?php echo smarty_modifier_truncate(str_replace('www.','',$_smarty_tpl->tpl_vars['_route_label']->value),23,'...',false,true);?>

                        </span>
                        <div class="hint bold js-hint nowrap"><?php if ($_smarty_tpl->tpl_vars['route']->value){?><?php echo $_smarty_tpl->tpl_vars['theme']->value['name'];?>
<?php }?></div>
                    </a>
                    <div class="dropdown-body js-design-actions">

                        <ul id="wa-theme-list" class="menu js-theme-list">
                            <?php  $_smarty_tpl->tpl_vars['r'] = new Smarty_Variable; $_smarty_tpl->tpl_vars['r']->_loop = false;
 $_from = $_smarty_tpl->tpl_vars['themes_routes']->value; if (!is_array($_from) && !is_object($_from)) { settype($_from, 'array');}
 $_smarty_tpl->tpl_vars['r']->iteration=0;
foreach ($_from as $_smarty_tpl->tpl_vars['r']->key => $_smarty_tpl->tpl_vars['r']->value){
$_smarty_tpl->tpl_vars['r']->_loop = true;
 $_smarty_tpl->tpl_vars['r']->iteration++;
?>
                                <?php if (is_array($_smarty_tpl->tpl_vars['r']->value)){?>
                                    
                                    <?php $_smarty_tpl->tpl_vars['_theme'] = new Smarty_variable($_smarty_tpl->tpl_vars['themes']->value[$_smarty_tpl->tpl_vars['r']->value['theme']], null, 0);?>
                                    <?php $_smarty_tpl->tpl_vars['_theme_is_trial'] = new Smarty_variable(($_smarty_tpl->tpl_vars['_theme']->value['type']==waTheme::TRIAL), null, 0);?>
                                    <?php ob_start();?><?php echo urlencode($_smarty_tpl->tpl_vars['r']->value['_domain']);?>
<?php $_tmp1=ob_get_clean();?><?php $_smarty_tpl->tpl_vars['_url'] = new Smarty_variable(((string)$_smarty_tpl->tpl_vars['design_url']->value)."theme=".((string)$_smarty_tpl->tpl_vars['r']->value['theme'])."&domain=".$_tmp1."&route=".((string)$_smarty_tpl->tpl_vars['r']->value['_id']), null, 0);?>
                                    <?php $_smarty_tpl->tpl_vars['_r_label'] = new Smarty_variable((waIdna::dec($_smarty_tpl->tpl_vars['r']->value['_domain'])).('/').($_smarty_tpl->tpl_vars['r']->value['url']), null, 0);?>
                                    <?php $_smarty_tpl->tpl_vars['_title'] = new Smarty_variable(str_replace('www.','',$_smarty_tpl->tpl_vars['_r_label']->value), null, 0);?>

                                    <li class="no-icon 2<?php if ($_smarty_tpl->tpl_vars['_theme_is_trial']->value){?> is-trial<?php }?><?php if ($_smarty_tpl->tpl_vars['current_url']->value==$_smarty_tpl->tpl_vars['_url']->value){?> selected<?php }?>"
                                        <?php if (!empty($_smarty_tpl->tpl_vars['r']->value['_routing_url'])){?> data-routing="<?php echo $_smarty_tpl->tpl_vars['r']->value['_routing_url'];?>
"<?php }?>
                                        data-id="<?php echo htmlspecialchars((string)$_smarty_tpl->tpl_vars['_theme']->value['id'], ENT_QUOTES, 'UTF-8', true);?>
">
                                        <a<?php if ($_smarty_tpl->tpl_vars['r']->iteration===1){?> class="js-is-first"<?php }?> href="<?php echo $_smarty_tpl->tpl_vars['_url']->value;?>
" title="<?php echo $_smarty_tpl->tpl_vars['_title']->value;?>
" data-preview-label="Сайт" data-url="<?php echo htmlspecialchars((string)$_smarty_tpl->tpl_vars['r']->value['_preview_url'], ENT_QUOTES, 'UTF-8', true);?>
">
                                        <span class="bold js-url">
                                            <?php echo smarty_modifier_truncate($_smarty_tpl->tpl_vars['_title']->value,45,'...',false,true);?>

                                        </span>
                                        <span class="count js-hint"><?php echo htmlspecialchars((string)$_smarty_tpl->tpl_vars['_theme']->value['name'], ENT_QUOTES, 'UTF-8', true);?>
</span>
                                        </a>
                                    </li>

                                    
                                    <?php if (!empty($_smarty_tpl->tpl_vars['r']->value['theme_mobile'])&&($_smarty_tpl->tpl_vars['r']->value['theme_mobile']!=$_smarty_tpl->tpl_vars['r']->value['theme'])&&!empty($_smarty_tpl->tpl_vars['themes']->value[$_smarty_tpl->tpl_vars['r']->value['theme_mobile']])){?>
                                        <?php $_smarty_tpl->tpl_vars['_theme'] = new Smarty_variable($_smarty_tpl->tpl_vars['themes']->value[$_smarty_tpl->tpl_vars['r']->value['theme_mobile']], null, 0);?>
                                        <?php $_smarty_tpl->tpl_vars['_theme_is_trial'] = new Smarty_variable(($_smarty_tpl->tpl_vars['_theme']->value['type']==waTheme::TRIAL), null, 0);?>
                                        <?php ob_start();?><?php echo urlencode($_smarty_tpl->tpl_vars['r']->value['_domain']);?>
<?php $_tmp2=ob_get_clean();?><?php $_smarty_tpl->tpl_vars['_url'] = new Smarty_variable(((string)$_smarty_tpl->tpl_vars['design_url']->value)."theme=".((string)$_smarty_tpl->tpl_vars['r']->value['theme_mobile'])."&domain=".$_tmp2."&route=".((string)$_smarty_tpl->tpl_vars['r']->value['_id']), null, 0);?>
                                        <?php $_smarty_tpl->tpl_vars['_r_label'] = new Smarty_variable((waIdna::dec($_smarty_tpl->tpl_vars['r']->value['_domain'])).('/').($_smarty_tpl->tpl_vars['r']->value['url']), null, 0);?>
                                        <?php $_smarty_tpl->tpl_vars['_title'] = new Smarty_variable(str_replace('www.','',$_smarty_tpl->tpl_vars['_r_label']->value), null, 0);?>

                                        <li class="no-icon 3<?php if ($_smarty_tpl->tpl_vars['_theme_is_trial']->value){?> is-trial<?php }?><?php if ($_smarty_tpl->tpl_vars['current_url']->value==$_smarty_tpl->tpl_vars['_url']->value){?> selected<?php }?>" <?php if (!empty($_smarty_tpl->tpl_vars['r']->value['_routing_url'])){?>data-routing="<?php echo $_smarty_tpl->tpl_vars['r']->value['_routing_url'];?>
"<?php }?> data-id="<?php echo htmlspecialchars((string)$_smarty_tpl->tpl_vars['_theme']->value['id'], ENT_QUOTES, 'UTF-8', true);?>
">
                                            <a href="<?php echo $_smarty_tpl->tpl_vars['_url']->value;?>
" title="<?php echo $_smarty_tpl->tpl_vars['_title']->value;?>
" data-preview-label="Предпросмотр"<?php if (!empty($_smarty_tpl->tpl_vars['_theme']->value['preview_url'])){?> data-url="<?php echo htmlspecialchars((string)$_smarty_tpl->tpl_vars['_theme']->value['preview_url'], ENT_QUOTES, 'UTF-8', true);?>
"<?php }?>>
                                                <span class="count">
                                                    <i class="fas fa-mobile-alt"></i>
                                                </span>
                                                <span class="bold js-url">
                                                    <?php echo smarty_modifier_truncate($_smarty_tpl->tpl_vars['_title']->value,45,'...',false,true);?>

                                                </span>
                                                <span class="count js-hint"><?php echo htmlspecialchars((string)$_smarty_tpl->tpl_vars['_theme']->value['name'], ENT_QUOTES, 'UTF-8', true);?>
</span>
                                            </a>
                                        </li>
                                    <?php }?>
                                <?php }else{ ?>
                                    
                                    <?php $_smarty_tpl->tpl_vars['_theme'] = new Smarty_variable($_smarty_tpl->tpl_vars['themes']->value[$_smarty_tpl->tpl_vars['r']->value], null, 0);?>
                                    <?php $_smarty_tpl->tpl_vars['_theme_is_trial'] = new Smarty_variable(($_smarty_tpl->tpl_vars['_theme']->value['type']==waTheme::TRIAL), null, 0);?>
                                    <?php $_smarty_tpl->tpl_vars['_url'] = new Smarty_variable(((string)$_smarty_tpl->tpl_vars['design_url']->value)."theme=".((string)$_smarty_tpl->tpl_vars['_theme']->value['id']), null, 0);?>
                                    <li class="no-icon 1<?php if ($_smarty_tpl->tpl_vars['_theme_is_trial']->value){?> is-trial<?php }?><?php if ($_smarty_tpl->tpl_vars['current_url']->value==$_smarty_tpl->tpl_vars['_url']->value){?> selected<?php }?>" data-id="<?php echo htmlspecialchars((string)$_smarty_tpl->tpl_vars['_theme']->value['id'], ENT_QUOTES, 'UTF-8', true);?>
">
                                        <a href="<?php echo $_smarty_tpl->tpl_vars['design_url']->value;?>
theme=<?php echo $_smarty_tpl->tpl_vars['_theme']->value['id'];?>
" data-preview-label="Предпросмотр"<?php if (!empty($_smarty_tpl->tpl_vars['_theme']->value['preview_url'])){?> data-url="<?php echo htmlspecialchars((string)$_smarty_tpl->tpl_vars['_theme']->value['preview_url'], ENT_QUOTES, 'UTF-8', true);?>
"<?php }?>>
                                            <span class="js-url"><?php echo htmlspecialchars((string)$_smarty_tpl->tpl_vars['_theme']->value['name'], ENT_QUOTES, 'UTF-8', true);?>
</span>
                                        </a>
                                    </li>
                                <?php }?>
                            <?php } ?>
                        </ul>
                        <?php if ($_smarty_tpl->tpl_vars['wa']->value->user()->getRights('installer')){?>
                            <div class="box custom-mt-0" data-is-nav>
                                <a class="wa-themes-link button webasyst-magic-wand full-width" href="<?php echo $_smarty_tpl->tpl_vars['themes_url']->value;?>
">
                                    <i class="icon"></i>
                                    <span>Все темы дизайна</span>
                                </a>
                            </div>
                            <div class="box custom-mt-0">
                                <a href="#" class="js-theme-upload-link button nobutton smaller nowrap">
                                    <span>Загрузить архив с темой</span>
                                    <span class="hint">(.tar.gz)</span>
                                </a>
                            </div>
                        <?php }?>
                    </div>
                </div>

                <ul class="tabs custom-p-0 js-tabs-menu js-design-actions bordered-bottom small">
                    <li data-action="theme" class="info selected custom-pb-0" data-is-nav>
                        <a class="wa-themes-link semibold" href="<?php echo $_smarty_tpl->tpl_vars['design_url']->value;?>
theme=<?php echo $_smarty_tpl->tpl_vars['theme']->value['id'];?>
&action=theme">
                            <svg width="14" height="14" viewBox="0 0 14 14" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
                                <path d="M1.3125 1.3125C1.05291 1.3125 0.799154 1.38948 0.583315 1.5337C0.367475 1.67792 0.199249 1.8829 0.0999087 2.12273C0.000568688 2.36256 -0.0254232 2.62646 0.0252199 2.88106C0.075863 3.13566 0.200867 3.36952 0.384423 3.55308C0.567979 3.73663 0.801845 3.86164 1.05644 3.91228C1.31104 3.96292 1.57494 3.93693 1.81477 3.83759C2.0546 3.73825 2.25959 3.57003 2.4038 3.35419C2.54802 3.13835 2.625 2.88459 2.625 2.625C2.625 2.2769 2.48672 1.94306 2.24058 1.69692C1.99444 1.45078 1.6606 1.3125 1.3125 1.3125ZM1.3125 5.6875C1.05291 5.6875 0.799154 5.76448 0.583315 5.9087C0.367475 6.05292 0.199249 6.2579 0.0999087 6.49773C0.000568688 6.73756 -0.0254232 7.00146 0.0252199 7.25606C0.075863 7.51066 0.200867 7.74452 0.384423 7.92808C0.567979 8.11163 0.801845 8.23664 1.05644 8.28728C1.31104 8.33792 1.57494 8.31193 1.81477 8.21259C2.0546 8.11325 2.25959 7.94503 2.4038 7.72919C2.54802 7.51335 2.625 7.25959 2.625 7C2.625 6.6519 2.48672 6.31806 2.24058 6.07192C1.99444 5.82578 1.6606 5.6875 1.3125 5.6875ZM1.3125 10.0625C1.05291 10.0625 0.799154 10.1395 0.583315 10.2837C0.367475 10.4279 0.199249 10.6329 0.0999087 10.8727C0.000568688 11.1126 -0.0254232 11.3765 0.0252199 11.6311C0.075863 11.8857 0.200867 12.1195 0.384423 12.3031C0.567979 12.4866 0.801845 12.6116 1.05644 12.6623C1.31104 12.7129 1.57494 12.6869 1.81477 12.5876C2.0546 12.4883 2.25959 12.32 2.4038 12.1042C2.54802 11.8883 2.625 11.6346 2.625 11.375C2.625 11.0269 2.48672 10.6931 2.24058 10.4469C1.99444 10.2008 1.6606 10.0625 1.3125 10.0625ZM13.5625 10.5H4.8125C4.69647 10.5 4.58519 10.5461 4.50314 10.6281C4.42109 10.7102 4.375 10.8215 4.375 10.9375V11.8125C4.375 11.9285 4.42109 12.0398 4.50314 12.1219C4.58519 12.2039 4.69647 12.25 4.8125 12.25H13.5625C13.6785 12.25 13.7898 12.2039 13.8719 12.1219C13.9539 12.0398 14 11.9285 14 11.8125V10.9375C14 10.8215 13.9539 10.7102 13.8719 10.6281C13.7898 10.5461 13.6785 10.5 13.5625 10.5ZM13.5625 1.75H4.8125C4.69647 1.75 4.58519 1.79609 4.50314 1.87814C4.42109 1.96019 4.375 2.07147 4.375 2.1875V3.0625C4.375 3.17853 4.42109 3.28981 4.50314 3.37186C4.58519 3.45391 4.69647 3.5 4.8125 3.5H13.5625C13.6785 3.5 13.7898 3.45391 13.8719 3.37186C13.9539 3.28981 14 3.17853 14 3.0625V2.1875C14 2.07147 13.9539 1.96019 13.8719 1.87814C13.7898 1.79609 13.6785 1.75 13.5625 1.75ZM13.5625 6.125H4.8125C4.69647 6.125 4.58519 6.17109 4.50314 6.25314C4.42109 6.33519 4.375 6.44647 4.375 6.5625V7.4375C4.375 7.55353 4.42109 7.66481 4.50314 7.74686C4.58519 7.82891 4.69647 7.875 4.8125 7.875H13.5625C13.6785 7.875 13.7898 7.82891 13.8719 7.74686C13.9539 7.66481 14 7.55353 14 7.4375V6.5625C14 6.44647 13.9539 6.33519 13.8719 6.25314C13.7898 6.17109 13.6785 6.125 13.5625 6.125Z"/>
                            </svg>
                            Оформление
                        </a>
                    </li>

                    <li data-action="edit" class="templates custom-pb-0" style="display: none;" data-is-nav>
                        <a href="<?php echo $_smarty_tpl->tpl_vars['design_url']->value;?>
theme=<?php echo $_smarty_tpl->tpl_vars['theme']->value['id'];?>
&action=edit" class="semibold">
                            <svg width="18" height="14" viewBox="0 0 18 14" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
                                <path d="M7.87612 13.9858L6.20815 13.5018C6.03315 13.4526 5.93472 13.2694 5.98394 13.0944L9.71636 0.237362C9.76558 0.0623624 9.94878 -0.0360751 10.1238 0.0131437L11.7917 0.497128C11.9667 0.546347 12.0652 0.72955 12.016 0.90455L8.28354 13.7616C8.23159 13.9366 8.05112 14.0378 7.87612 13.9858ZM4.75893 10.9178L5.94839 9.64908C6.07417 9.5151 6.06597 9.30182 5.92651 9.17877L3.44917 6.99947L5.92651 4.82018C6.06597 4.69713 6.0769 4.48385 5.94839 4.34986L4.75893 3.08111C4.63589 2.94986 4.42808 2.94166 4.29409 3.06744L0.353857 6.75885C0.214404 6.88736 0.214404 7.10885 0.353857 7.23736L4.29409 10.9315C4.42808 11.0573 4.63589 11.0518 4.75893 10.9178ZM13.7058 10.9342L17.646 7.2401C17.7855 7.11158 17.7855 6.8901 17.646 6.76158L13.7058 3.06471C13.5746 2.94166 13.3667 2.94713 13.241 3.07838L12.0515 4.34713C11.9257 4.48111 11.9339 4.69439 12.0734 4.81744L14.5507 6.99947L12.0734 9.17877C11.9339 9.30182 11.923 9.5151 12.0515 9.64908L13.241 10.9178C13.364 11.0518 13.5718 11.0573 13.7058 10.9342Z"/>
                            </svg>
                            Шаблоны
                        </a>
                    </li>
                    
                </ul>
            </div>
            <div class="sidebar-body blank">
                <div class="js-tab-content tab-content flexbox vertical height-100 selected" data-id="theme">

                    <section class="wa-theme-settings js-theme-settings-section">
                        <?php smarty_template_function_skeleton_sidebar($_smarty_tpl,array());?>

                    </section>

                    <div class="box align-center">
                        <a href="javascript:void(0)" class="button light-gray rounded small js-theme-info-link">
                            <span>Информация о теме</span>
                        </a>
                    </div>

                    <div class="js-design-actions box align-center">
                        <div data-action="settings" class="settings" data-is-nav>
                            <a href="<?php echo $_smarty_tpl->tpl_vars['design_url']->value;?>
action=settings" class="button light-gray rounded small">
                                <i class="fas fa-cog text-blue custom-mr-4"></i>
                                <span><?php if ($_smarty_tpl->tpl_vars['_route_label']->value){?><?php echo sprintf('Настройки %s',"<span class=\"js-route-label\">".((string)$_smarty_tpl->tpl_vars['_route_label']->value)."</span>");?>
<?php }else{ ?>Настройки маршрутизации<?php }?></span>
                            </a>
                        </div>
                    </div>
                </div>
                <div id="wa-design-sidebar" class="js-tab-content tab-content" data-id="edit">
                    <div class="wa-theme-block custom-mt-0">
                        <?php if ($_smarty_tpl->tpl_vars['edit_data']->value['theme_usages']&&$_smarty_tpl->tpl_vars['edit_data']->value['route_url']){?>
                            <div class="alert warning small custom-mx-16 custom-mt-16">
                                <i class="fas fa-exclamation-triangle fa-sm"></i> Эта тема дизайна также используется в других разделах ваших сайтов. Редактирование ее шаблонов дизайна приведет к изменению внешнего вида всех этих разделов.
                            </div>
                        <?php }?>
                        <div class="search-input custom-mx-12 custom-mt-16">
                            <input type="search" class="js-theme-files-search small width-100" placeholder="Поиск">
                        </div>
                        <ul class="menu ellipsis wa-theme-templates">
                            <?php if (!function_exists('smarty_template_function_renderFileTree')) {
    function smarty_template_function_renderFileTree($_smarty_tpl,$params) {
    $saved_tpl_vars = $_smarty_tpl->tpl_vars;
    foreach ($_smarty_tpl->smarty->template_functions['renderFileTree']['parameter'] as $key => $value) {$_smarty_tpl->tpl_vars[$key] = new Smarty_variable($value);};
    foreach ($params as $key => $value) {$_smarty_tpl->tpl_vars[$key] = new Smarty_variable($value);}?>
                                <?php $_smarty_tpl->tpl_vars['_level_padding'] = new Smarty_variable("calc(0.55rem * ".((string)$_smarty_tpl->tpl_vars['level']->value)." + 0.75rem)", null, 0);?>
                                <?php  $_smarty_tpl->tpl_vars['f'] = new Smarty_Variable; $_smarty_tpl->tpl_vars['f']->_loop = false;
 $_smarty_tpl->tpl_vars['f_id'] = new Smarty_Variable;
 $_from = $_smarty_tpl->tpl_vars['theme_files']->value; if (!is_array($_from) && !is_object($_from)) { settype($_from, 'array');}
 $_smarty_tpl->tpl_vars['f']->index=-1;
foreach ($_from as $_smarty_tpl->tpl_vars['f']->key => $_smarty_tpl->tpl_vars['f']->value){
$_smarty_tpl->tpl_vars['f']->_loop = true;
 $_smarty_tpl->tpl_vars['f_id']->value = $_smarty_tpl->tpl_vars['f']->key;
 $_smarty_tpl->tpl_vars['f']->index++;
 $_smarty_tpl->tpl_vars['f']->first = $_smarty_tpl->tpl_vars['f']->index === 0;
?>
                                    <?php if ($_smarty_tpl->tpl_vars['f']->value['type']=='dir'){?>
                                        <?php $_smarty_tpl->tpl_vars['current_dir_path'] = new Smarty_variable(((string)$_smarty_tpl->tpl_vars['parent_path']->value).((string)$_smarty_tpl->tpl_vars['f_id']->value)."/", null, 0);?>
                                        <?php $_smarty_tpl->tpl_vars['is_expanded'] = new Smarty_variable((!empty($_smarty_tpl->tpl_vars['edit_data']->value['file']['id'])&&strpos($_smarty_tpl->tpl_vars['edit_data']->value['file']['id'],$_smarty_tpl->tpl_vars['current_dir_path']->value)===0), null, 0);?>

                                        <li>
                                            <span class="item js-directory-toggle"<?php if ($_smarty_tpl->tpl_vars['level']->value>0){?> style="padding-left:<?php echo $_smarty_tpl->tpl_vars['_level_padding']->value;?>
;"<?php }?>>
                                                <i class="fas fa-folder"></i>
                                                <span><?php echo htmlspecialchars((string)$_smarty_tpl->tpl_vars['f_id']->value, ENT_QUOTES, 'UTF-8', true);?>
</span>
                                            </span>
                                            <ul class="menu ellipsis"<?php if (!$_smarty_tpl->tpl_vars['is_expanded']->value){?> style="display: none;"<?php }?>>
                                                <?php smarty_template_function_renderFileTree($_smarty_tpl,array('theme_files'=>$_smarty_tpl->tpl_vars['f']->value['children'],'level'=>$_smarty_tpl->tpl_vars['level']->value+1,'parent_path'=>$_smarty_tpl->tpl_vars['current_dir_path']->value));?>

                                            </ul>
                                        </li>
                                    <?php }else{ ?>
                                        <li class="<?php if ($_smarty_tpl->tpl_vars['f']->value['path']==$_smarty_tpl->tpl_vars['edit_data']->value['file']['id']){?>selected <?php }?><?php if ($_smarty_tpl->tpl_vars['f']->first&&$_smarty_tpl->tpl_vars['level']->value==0){?>top-padded <?php }?><?php if ($_smarty_tpl->tpl_vars['f']->value['parent']){?><?php if (!empty($_smarty_tpl->tpl_vars['f']->value['parent_exists'])){?>inherit<?php }else{ ?>error<?php }?><?php }?>" data-filename="<?php echo htmlspecialchars((string)$_smarty_tpl->tpl_vars['f_id']->value, ENT_QUOTES, 'UTF-8', true);?>
">
                                            <a <?php if (!empty($_smarty_tpl->tpl_vars['f']->value['modified'])){?>class="bold"<?php }?> href="<?php echo $_smarty_tpl->tpl_vars['design_url']->value;?>
theme=<?php echo $_smarty_tpl->tpl_vars['theme']->value['id'];?>
&file=<?php echo rawurlencode($_smarty_tpl->tpl_vars['f']->value['path']);?>
" title="<?php if (!empty($_smarty_tpl->tpl_vars['f']->value['parent_exists'])){?>файл родительской темы: <?php }?><?php echo htmlspecialchars((string)$_smarty_tpl->tpl_vars['f']->value['path'], ENT_QUOTES, 'UTF-8', true);?>
"<?php if ($_smarty_tpl->tpl_vars['level']->value>0){?> style="padding-left:<?php echo $_smarty_tpl->tpl_vars['_level_padding']->value;?>
;"<?php }?>>
                                                <i class="<?php if (substr($_smarty_tpl->tpl_vars['f']->value['path'],-4)=='.css'){?>fab fa-css3<?php }elseif(substr($_smarty_tpl->tpl_vars['f']->value['path'],-3)=='.js'){?>fab fa-js-square<?php }else{ ?>fas fa-file-code<?php }?>"></i>
                                                <span><?php echo htmlspecialchars((string)$_smarty_tpl->tpl_vars['f_id']->value, ENT_QUOTES, 'UTF-8', true);?>
</span>
                                                <span class="count small"><?php if (!empty($_smarty_tpl->tpl_vars['f']->value['parent_exists'])){?><i class="fas fa-link" title="<?php if (!empty($_smarty_tpl->tpl_vars['f']->value['parent_exists'])){?>файл родительской темы<?php }?>"></i><?php }?></span>
                                            </a>
                                        </li>
                                    <?php }?>
                                <?php } ?>
                            <?php $_smarty_tpl->tpl_vars = $saved_tpl_vars;
foreach (Smarty::$global_tpl_vars as $key => $value) if(!isset($_smarty_tpl->tpl_vars[$key])) $_smarty_tpl->tpl_vars[$key] = $value;}}?>

                            <?php smarty_template_function_renderFileTree($_smarty_tpl,array('theme_files'=>$_smarty_tpl->tpl_vars['edit_data']->value['theme_files_tree']));?>

                            <li class="top-padded">
                                <a class="small" href="<?php echo $_smarty_tpl->tpl_vars['design_url']->value;?>
theme=<?php echo $_smarty_tpl->tpl_vars['theme']->value['id'];?>
&file=" title="Новый файл">
                                    <i class="fas fa-plus-circle"></i>
                                    <span>Новый файл</span>
                                </a>
                            </li>
                        </ul>
                    </div>
                </div>
            </div>
        </div>
        <?php if (!function_exists('smarty_template_function_skeleton')) {
    function smarty_template_function_skeleton($_smarty_tpl,$params) {
    $saved_tpl_vars = $_smarty_tpl->tpl_vars;
    foreach ($_smarty_tpl->smarty->template_functions['skeleton']['parameter'] as $key => $value) {$_smarty_tpl->tpl_vars[$key] = new Smarty_variable($value);};
    foreach ($params as $key => $value) {$_smarty_tpl->tpl_vars[$key] = new Smarty_variable($value);}?>
            <div class="not-blank skeleton article">
                <div class="article-body">
                    <?php $_smarty_tpl->tpl_vars['i'] = new Smarty_Variable;$_smarty_tpl->tpl_vars['i']->step = 1;$_smarty_tpl->tpl_vars['i']->total = (int)ceil(($_smarty_tpl->tpl_vars['i']->step > 0 ? 5+1 - (1) : 1-(5)+1)/abs($_smarty_tpl->tpl_vars['i']->step));
if ($_smarty_tpl->tpl_vars['i']->total > 0){
for ($_smarty_tpl->tpl_vars['i']->value = 1, $_smarty_tpl->tpl_vars['i']->iteration = 1;$_smarty_tpl->tpl_vars['i']->iteration <= $_smarty_tpl->tpl_vars['i']->total;$_smarty_tpl->tpl_vars['i']->value += $_smarty_tpl->tpl_vars['i']->step, $_smarty_tpl->tpl_vars['i']->iteration++){
$_smarty_tpl->tpl_vars['i']->first = $_smarty_tpl->tpl_vars['i']->iteration == 1;$_smarty_tpl->tpl_vars['i']->last = $_smarty_tpl->tpl_vars['i']->iteration == $_smarty_tpl->tpl_vars['i']->total;?>
                        <span class="skeleton-header"></span>
                        <span class="skeleton-line"></span>
                        <span class="skeleton-line"></span>
                        <span class="skeleton-line"></span>
                    <?php }} ?>
                </div>
            </div>
        <?php $_smarty_tpl->tpl_vars = $saved_tpl_vars;
foreach (Smarty::$global_tpl_vars as $key => $value) if(!isset($_smarty_tpl->tpl_vars[$key])) $_smarty_tpl->tpl_vars[$key] = $value;}}?>

        <!-- theme navigation -->
        <div id="wa-design-container" class="content not-blank js-design-container">
            <?php smarty_template_function_skeleton($_smarty_tpl,array());?>

        </div>

        <div class="dialog" id="wa-design-preview">
            <div class="dialog-background"></div>
            <form class="dialog-body">
                <h3 class="dialog-header custom-pb-16">Предварительный просмотр</h3>
                <div class="dialog-content custom-pb-8">
                    <input type="hidden" id="wa-preview-key" value="" />
                    <p class="custom-mt-4"><i class="fas fa-check-circle text-green"></i> Тема дизайна «<strong id="wa-current-theme"></strong>» временно активирована для вашего пользователя. Вы можете полноценно просматривать сайт в выбранной теме дизайна, и другие посетители сайта этого не увидят.</p>
                    <?php if (waRequest::isHttps()){?>
                        <p>Предварительный просмотр темы дизайна не будет работать на доменном имени <strong id="wa-preview-domain"></strong>, если оно недоступно по протоколу HTTPS. Если это так, то для предпросмотра темы дизайна либо установите SSL-сертификат для этого домена, либо войдите в свой Webasyst на этом домене.</p>
                    <?php }?>
                    <?php if ($_smarty_tpl->tpl_vars['theme']->value['preview_url']){?>
                        <a href="<?php echo htmlspecialchars((string)$_smarty_tpl->tpl_vars['theme']->value['preview_url'], ENT_QUOTES, 'UTF-8', true);?>
" target="_blank" class="button green">
                            <span>Открыть предпросмотр на сайте</span><i class="fas fa-external-link-alt smaller custom-ml-4 opacity-50"></i>
                        </a>
                    <?php }?>
                </div>
                <div class="dialog-footer">
                    <input type="submit" class="button outlined orange" value="Завершить предварительный просмотр">
                </div>
            </form>
        </div>

        <div class="dialog" id="wa-theme-upload-dialog">
                <div class="dialog-background"> </div>
                <form class="dialog-body" id="wa-theme-upload-form" method="post" action="?module=<?php echo $_smarty_tpl->tpl_vars['module_name']->value;?>
&amp;action=themeUpload" enctype="multipart/form-data">
                    <h3 class="dialog-header">Загрузить тему</h3>
                    <div class="dialog-content">
                        <span class="wa-theme-dialog-error bold" style="color: red;"></span>
                        <p>Загружаемая тема должна представлять собой правильный архив темы Webasyst (архив в формате .tar.gz с файлами темы и файлом-манифестом theme.xml).</p>
                        <div class="upload-area">
                            <div class="upload">
                                <label class="link">
                                    <i class="fas fa-file-upload"></i>
                                    <span>Выберите файл</span>
                                    <input id="wa-input-file" type="file" name="theme_files[]" autocomplete="off">
                                </label>
                            </div>
                        </div>
                        <?php echo $_smarty_tpl->tpl_vars['wa']->value->csrf();?>

                        <div class="loading" style="display:none; margin-top: 10px">
                            <i class="fas fa-spinner fa-spin"></i> Загрузка...
                        </div>
                    </div>
                    <div class="dialog-footer">
                        <input type="submit" class="button green" value="Загрузить">
                        <a href="<?php echo $_smarty_tpl->tpl_vars['themes_url']->value;?>
" class="js-close-dialog button light-gray">Отмена</a>
                    </div>
                </form>
            </div>

        <script type="text/javascript">
            var skeleton_template = <?php ob_start();?><?php smarty_template_function_skeleton($_smarty_tpl,array());?>
<?php echo json_encode(ob_get_clean())?>,
                skeleton_sidebar = <?php ob_start();?><?php smarty_template_function_skeleton_sidebar($_smarty_tpl,array());?>
<?php echo json_encode(ob_get_clean())?>,
                $design_actions = $('.js-design-actions').find('[data-is-nav]'),
                $design_container = $('.js-design-container'),
                $theme_list = $('.js-theme-list'),
                $theme_settings_section = $('.js-theme-settings-section'),
                $theme_preview_link = $('.js-tabs-menu .wa-theme-preview'),
                $theme_edit_link = $('.js-tabs-menu li[data-action="edit"]');

            var $bottombar = $('.bottombar');

            async function postData(url, data) {
                const response = await fetch(url, {
                    method: 'POST',
                    body: data,
                });
                return await response.text();
            }

            function waDesignConfirm() {
                if ($("#wa-design-button").length && $('#wa-design-button').hasClass('yellow') || $("#bb_submit").length && $('#bb_submit').hasClass('yellow') ) {
                    if (!confirm("Несохраненные изменения будут потеряны. Продолжить?")) {
                        return false;
                    }
                }
                return true;
            }
            $(window).on('beforeunload', function () {
                const $design_button = $("#wa-design-button");
                if ($design_button.length && $design_button.hasClass('yellow')) {
                    return "Несохраненные изменения будут потеряны. Продолжить?";
                }
            });

            /* Make Dropdown for Theme select menu */
            var $theme_select_menu = $('.js-theme-select-menu').waDropdown();

            /* Switch between Appearance, Templates, Settings and Themes displays */
            $design_actions.on('click', function (e) {
                if(this.classList.contains('selected')) {
                    return;
                }

                let $tab = $(this),
                    tab_action = $tab.data('action'),
                    $tab_content = $(`.js-tab-content[data-id="${ tab_action }"]`);

                $tab.addClass('selected');
                $tab_content.addClass('selected').siblings().removeClass('selected');

                if(!tab_action){
                    $theme_select_menu.removeClass('is-opened');
                }

                if (tab_action === 'settings') {
                    let $theme_settings_list = $('.js-theme-settings-list');

                    if ($theme_settings_list.length) {
                        $theme_settings_list.find('li').removeClass('selected');
                    }
                }

                <?php if (!$_smarty_tpl->tpl_vars['options']->value['is_ajax']){?>
                    if ($(this).is('li')){
                        waDesignLoad($(this).find('a').attr('href')?.replace(/.*?#\//, '')?.replace(/\/$/, ''));
                    }else{
                        waDesignLoad($(this).attr('href')?.replace(/.*?#\//, '')?.replace(/\/$/, ''));
                    }
                <?php }?>
            });

            /* Switch theme settings menu */
            $theme_settings_section.on('click', '.js-theme-settings-list a', function (e, id) {
                let $self = $(this),
                    $li = $self.parent(),
                    $theme_info_section = $('.js-theme-info-section'),
                    menu_id = $self.data('divider-id'),
                    $settings_section = $('.js-settings-list'),
                    $settings_section_selected = $settings_section.find(`[data-divider-id="${ menu_id }"]`);

                let hash = waDesignHash(location.hash)
                if (hash.length) {
                    if (hash == 'themes' || !!~hash.indexOf('action=settings')) {
                        let hash = $design_actions.filter('[data-action="theme"]').find('a').attr('href')
                        $.wa.setHash(hash);
                        waDesignLoad(hash.replace(/.*?#\//, '').replace(/\/$/, ''));
                        $theme_settings_section.data('menu-id', menu_id)
                    }
                }

                $li.addClass('selected').siblings().removeClass('selected');

                if (!$bottombar.length) {
                    $bottombar = $('.bottombar');
                }

                $(document).on('wa_design_loaded', function () {
                    $bottombar = $('.bottombar');
                    if (menu_id >= 0) {
                        $bottombar.show();
                    } else {
                        $bottombar.hide();
                    }
                })

                if (menu_id >= 0) {
                    $theme_info_section.hide();
                    $settings_section.show();
                    $settings_section_selected.addClass('selected').siblings().removeClass('selected');
                    $bottombar.show();
                    sessionStorage.setItem('wa_design_menu_id', menu_id);
                } else {
                    $settings_section.hide();
                    $theme_info_section.show();
                    $bottombar.hide();
                    sessionStorage.removeItem('wa_design_menu_id');
                }

                window.scrollTo({ top: 0, behavior: 'smooth' });
            });

            $(document).on('wa_design_loaded', function () {
                const current_menu_id = sessionStorage.getItem('wa_design_menu_id');
                if (current_menu_id) {
                    $theme_settings_section.find('a[data-divider-id='+current_menu_id+']').trigger('click');
                }
            });

            /**
             * TODO Remove when Site app will be ready for WA2
             * @param action
             */
            function reStyleSettingsPage(action) {
                if(action === 'settings') {
                    let $save_panel = $('.wa-design-save-panel'),
                        $design_container = $('.js-design-container'),
                        $design_container_field_group = $design_container.find('.field-group');

                    $design_container_field_group.removeClass('field-group').addClass('fields-group')

                    $design_container.append('<div class="bottombar sticky bordered-top box flexbox middle space-16"></div>');
                    let $design_container_bottombar = $design_container.find('.bottombar');

                    $save_panel
                        .appendTo($design_container_bottombar)
                        .removeClass('wa-design-save-panel block bordered-top')
                        .addClass('article width-100 flexbox middle space-16')
                        .find('[type="submit"]')
                        .attr('form', '#s-settings-form');

                    $bottombar = $design_container_bottombar;
                }
            }

            var waDesignHash = function (url) {
                if (!url || !url.length) {
                    return '';
                }
                url = url.replace(/\/$/, '').replace(/^#/, '')
                var urls = url.split('/');
                var n = urls.length;
                url = urls[n - 1];
                var i = n - 2;
                while (i >= 0 && urls[i].indexOf('=') !== -1) {
                    url = urls[i] + '/' + url;
                    i--;
                }
                return url;
            };

            var waDesignParams = function (url) {
                url = waDesignHash(url);
                var params_ar = url.split('&');
                var params = { };
                for (var i = 0; i < params_ar.length; i++) {
                    var tmp = params_ar[i].split('=');
                    params[tmp[0]] = tmp[1];
                }
                return params;
            };

            /* Select any theme into menu */
            $theme_list.find("li:not([data-is-nav]) a").on('click', function (e, set_active_only) {
                let $self = $(this),
                    href = $self.attr('href');

                $theme_select_menu.removeClass('is-opened');

                $design_container.html(skeleton_template);
                if (!set_active_only) {
                    $theme_settings_section.html(skeleton_sidebar);
                }
                $theme_list.find("li").removeClass('selected');

                $self.parent().addClass('selected');

                const params = waDesignParams(href);

                if (params['theme']) {
                    const $theme_active = $("#wa-theme-active"),
                        $theme_active_url = $theme_active.find('.js-url'),
                        $theme_active_hint = $theme_active.find('.js-hint');

                    $theme_active_url.html($self.find('.js-url').clone().children().remove().end().text());
                    if ($self.find('.js-hint').length) {
                        $theme_active_hint.html($self.find('.js-hint').html());
                    } else {
                        $theme_active_hint.empty();
                    }

                    let $menu_link = $('#wa-theme-active')
                    let menu_link_href = $theme_active.attr('href');
                    if (menu_link_href.indexOf('theme=') !== -1) {
                        $menu_link.attr('href', menu_link_href.replace(/theme=[^&]+/, 'theme=' + params['theme']));
                    }


                    $design_actions.find('a').each(function () {
                        let $menu_link = $(this);
                        if ($menu_link.parent().data('action')) {
                            $menu_link.attr('href', href + '&action=' + $menu_link.parent().data('action'));
                        } else {
                            let menu_link_href = $menu_link.attr('href');
                            if (menu_link_href.indexOf('theme=') !== -1) {
                                $menu_link.attr('href', menu_link_href.replace(/theme=[^&]+/, 'theme=' + params['theme']));
                            }
                        }
                    });

                    // Setup preview link
                    let preview_href = ($self.data('url')) ? $self.data('url') : $theme_preview_link.data('url');
                    $theme_preview_link.attr('href', preview_href).data('theme-id', params['theme']);
                    $theme_preview_link.find('span').text($self.data('preview-label'));

                    if (set_active_only) {
                        return false;
                    }
                    let hash = location.hash;
                    const ls_id = 'wa_<?php echo $_smarty_tpl->tpl_vars['app']->value['id'];?>
_current_theme_hash';
                    if (hash.indexOf('theme=') !== -1) {
                        if (!params['route']) {
                            hash = hash.replace(/&route=[^&]+/, '');
                        } else if (hash.indexOf('route=') !== -1) {
                            hash = hash.replace(/route=[^&]+/, 'route=' + params['route']);
                        } else {
                            hash = hash + '&route=' + params['route'];
                        }
                        if (!params['domain']) {
                            hash = hash.replace(/&domain=[^&]+/, '');
                        } else if (hash.indexOf('domain=') !== -1) {
                            hash = hash.replace(/domain=[^&]+/, 'domain=' + params['domain']);
                        } else {
                            hash = hash + '&domain=' + params['domain'];
                        }
                        $.wa.setHash(hash.replace(/theme=[^&]+/, 'theme=' + params['theme']));
                        localStorage.setItem(ls_id, location.hash);
                    } else {
                        const theme_hash = localStorage.getItem(ls_id);
                        if (theme_hash){
                            location.href = theme_hash;
                        }else{
                            location.href = href;
                        }
                    }

                    <?php if (!$_smarty_tpl->tpl_vars['options']->value['is_ajax']){?>waDesignLoad();<?php }?>
                    return false;
                }
            });

            /* Theme preview */
            $('.js-tabs-menu, .js-design-container').on('click auxclick', ".wa-theme-preview", function (e) {
                if (e.which === 3) {
                    return;
                }
                var $self = $(this),
                    theme_id = $self.data('theme-id'),
                    url = $self.attr('data-href') || $self.attr('href'),
                    $selected_href = $theme_list.find('li.selected a').attr('href'),
                    tmp_params = waDesignParams($selected_href);

                if (!tmp_params['route']) {
                    e.preventDefault();
                    var href = "?module=<?php echo $_smarty_tpl->tpl_vars['module_name']->value;?>
&action=setPreview",
                        data = {
                            theme_id: theme_id
                        };

                    $.post(href, data, function () {
                        window.open(url, "_blank");
                    });

                    $('#wa-current-theme').html($('#wa-theme-active .js-url').html());

                    // Warn user that preview only works when opened via HTTPS
                    // (when current domain opened via HTTPS and is different from target storefront domain)
                    var $https_warning = $('#wa-preview-domain');
                    if ($https_warning.length) {
                        var origin = $('<a>').attr('href', url)[0].origin;
                        if (origin == location.origin) {
                            $https_warning.closest('p').hide();
                        } else {
                            $https_warning.closest('p').show();
                            $https_warning.text(origin);
                        }
                    }

                    const $d_wrapper = $("#wa-design-preview").clone();
                    $.waDialog({
                        $wrapper: $d_wrapper,
                        onOpen($dialog, dialog) {
                            const $form = $dialog.find('form');

                            $form.on('submit', function (e){
                                e.preventDefault();
                                if (url.indexOf('?theme_hash') === -1) {
                                    url += '?theme_hash=&set_force_theme=';
                                }
                                $("body")
                                    .append($('<iframe style="display:none" src="' + url.replace(/(set_force_theme=).*$/, '$1') + '" />')
                                        .on('load', function () {
                                            $(this).remove();
                                        }));
                                dialog.close();
                            })
                        }
                    });
                }
            });

            var site_routing_full = function () { };
            var wa_design_not_load = false;

            function waDesignLoad(hash) {
                if(hash) {
                    hash = hash.replace('design/theme', 'design&theme');
                }

                <?php if ($_smarty_tpl->tpl_vars['design_url']->value&&is_string($_smarty_tpl->tpl_vars['design_url']->value)&&$_smarty_tpl->tpl_vars['design_url']->value!='#/'){?>
                var design_url = <?php echo json_encode($_smarty_tpl->tpl_vars['design_url']->value);?>
.replace(/.*?#\//, '');
                if (design_url && design_url.length && hash && hash.length) {
                    if (hash.substr(0, design_url.length) == design_url) {
                        hash = hash.substr(design_url.length);
                    }
                }
                <?php }?>
                var theme_is_trial = false,
                    hash_params = waDesignParams(hash || location.hash);

                if (hash_params['theme']) {
                    let $selected_theme_item = $theme_list.find('[data-id="'+ hash_params['theme'] +'"]');
                    if ($selected_theme_item.hasClass('is-trial')) {
                        theme_is_trial = true;
                        hash_params['action'] = 'theme';
                        $theme_edit_link.hide();
                        $theme_preview_link.addClass('is-trial');
                    } else {
                        $theme_edit_link.show();
                        $theme_preview_link.removeClass('is-trial');
                    }

                    let current_theme_url_build = 'theme=' + hash_params['theme'];
                    if (hash_params['domain']) {
                        current_theme_url_build += '&domain=' + hash_params['domain'];
                    }
                    if (hash_params['route']) {
                        current_theme_url_build += '&route=' + hash_params['route'];
                    }

                    let current_theme_url = $theme_list.find('[href="<?php echo $_smarty_tpl->tpl_vars['design_url']->value;?>
' + current_theme_url_build + '"]');
                    if (current_theme_url.length) {
                        current_theme_url.trigger('click', [true]);
                    } else {
                        $theme_list.find('[href^="<?php echo $_smarty_tpl->tpl_vars['design_url']->value;?>
theme=' + hash_params['theme'] + '"]:first').trigger('click', [true]);
                    }
                }

                let $selected_theme_list_li = $theme_list.find('.selected:not([data-is-nav])'),
                    $selected_theme_list_a = $selected_theme_list_li.find('a');

                if ($selected_theme_list_a.length) {
                    let params = waDesignParams($selected_theme_list_a.attr('href')),
                        data_routing = $selected_theme_list_li.data('routing'),
                        $settings_design_menu_li = $design_actions.filter('.settings'),
                        $settings_design_menu_a = $settings_design_menu_li.find('a');

                    if (params['route'] && data_routing) {
                        $settings_design_menu_a.attr('href', $selected_theme_list_a.attr('href') + '&action=settings');
                        $settings_design_menu_a.find('.js-route-label').text($selected_theme_list_a.find('.js-url').text());
                        $settings_design_menu_li.show();
                    } else {
                        $settings_design_menu_li.hide();
                    }
                }

                if (wa_design_not_load) {
                    wa_design_not_load = false;
                    return;
                }

                if (hash === undefined) {
                    hash = waDesignHash(location.hash);
                }

                if (hash.length) {

                    if (hash == 'themes') {
                        $design_actions.filter('.selected').removeClass('selected');
                        $design_actions.filter('.themes').addClass('selected');
                        //$theme_settings_section.empty();
                        $design_container.load("?module=<?php echo $_smarty_tpl->tpl_vars['module_name']->value;?>
&action=themes");
                    } else if (!!~hash.indexOf('action=settings')) {

                        let $selected_theme_list_li = $theme_list.find('.selected:not([data-is-nav])'),
                            $li = $theme_list.find(':not([data-is-nav]):first');

                        if ($selected_theme_list_li.length) {
                            $li = $selected_theme_list_li;
                        }

                        let hash_params = waDesignParams(hash);

                        if ($li.data('routing')) {
                            let li_route = $li.data('routing').replace(/.*route=([^&]+).*?/i, '$1');
                        if (hash_params['route'] && (hash_params['route'] != li_route)) {
                                let $tmp_li = $theme_list.find('[data-routing$="route=' + hash_params['route'] + '"]:first');
                                if ($tmp_li.length) {
                                    $tmp_li.find('a').click();
                                }
                            } else {
                                $design_container.load($li.data('routing') + '&reload_on_change=1&ui=2.0', function () {
                                    reStyleSettingsPage('settings');
                                    $design_container.find(".back").remove(); // deprecated?
                                    $(".s-route-core").hide();
                                    $('.s-route-details .fields').
                                        prepend($('<div class="fields-group"></div>').
                                        append($('<div class="field"><div class="name">URL</div></div>').
                                        append($('<div class="value"></div>').html($('#s-route-where').html()))
                                        )
                                    );
                                    $design_container.find(".s-route-delete").closest('div').remove();
                                });

                                const newThemeId = hash_params['theme'];
                                // Загружаем содержимое темы через GET запрос, чтобы извлечь меню
                                $.get("?module=design&action=theme&theme="+newThemeId+"&route="+li_route, function(html) {
                                    // Парсим HTML и извлекаем только меню
                                    const $theme_settings_list = $('<div>').html(html).find('.js-theme-settings-hidden-list');
                                    if ($theme_settings_list.length) {
                                        self.$theme_settings_section.html($theme_settings_list.html());
                                    }
                                    // Отмечаем, что содержимое загружено для текущей темы
                                    self.$theme_settings_section.data('loaded', true);
                                    self.$theme_settings_section.data('theme-id', newThemeId);

                                    $('.js-theme-settings-link').closest('li').addClass('selected');
                                });
                            }
                        } else {
                            if (hash_params['route']) {
                                let $tmp_li = $theme_list.find('[data-routing$="route=' + hash_params['route'] + '"]:first');
                                if ($tmp_li.length) {
                                    $tmp_li.find('a').click();
                                }
                            } else {
                                $.wa.setHash(location.hash.replace(/&?action=settings/, ''));
                            }
                        }
                    } else {
                        if (!~hash.indexOf('action=')) {
                            if (!~hash.indexOf('file=')) {
                                hash = 'action=theme&' + hash;
                            }else{
                                hash = 'action=edit&' + hash;
                            }
                        }

                        if (!!~hash.indexOf('theme=')) {
                            let params = hash.split('&'),
                                $theme_files_sidebar = $('.js-tab-content[data-id="edit"]');

                            for (var j = 0; j < params.length; j++) {
                                var tmp = params[j].split('=');

                                if (tmp[0] == 'theme') {
                                    $theme_files_sidebar.find('.selected').removeClass('selected')
                                    $theme_files_sidebar.find('[href="' + location.hash + '"]').closest('li').addClass('selected')
                                }
                            }
                        }

                        if (!!~hash.indexOf('action=edit')) {
                            $design_actions.filter('.selected').removeClass('selected');
                            if (theme_is_trial) {
                                hash = hash.replace('action=edit', 'action=theme');
                                $design_actions.filter('.info').addClass('selected');
                            } else {
                                $design_actions.filter('.templates').addClass('selected');
                                $('.js-tab-content').removeClass('selected');
                                $('.js-tab-content[data-id="edit"]').addClass('selected');
                                let _hash = hash.replace('action=edit', 'action=editFiles');
                                $("#wa-design-sidebar").load("?module=<?php echo $_smarty_tpl->tpl_vars['module_name']->value;?>
&action=editFiles&" + _hash);
                            }
                        } else if (!!~hash.indexOf('action=theme')) {
                            $design_actions.filter('.selected').removeClass('selected');
                            $design_actions.filter('.info').addClass('selected');
                        }
                        // Save theme settings
                        $design_container.load("?module=<?php echo $_smarty_tpl->tpl_vars['module_name']->value;?>
&domain_id=<?php echo htmlspecialchars((string)waRequest::request('domain_id','','int'), ENT_QUOTES, 'UTF-8', true);?>
&" + hash, function (responseText) {
                            $(document).trigger('wa_design_loaded', true)
                            try {
                                let response = $.parseJSON(responseText);
                                if (response) {
                                    if (response.data && response.data.redirect) {
                                        let href = location.href.replace(/#.*$/, '') + response.data.redirect;
                                        location.replace(href);
                                    }
                                }
                            } catch (e) {
                            }
                        });
                    }
                } else {
                    $theme_list.find("li:not([data-is-nav]) a:first").click();
                }
            }

            $(document).ready(function () {
                let $a = $('.js-tab-content .wa-theme-block a[href="' + location.hash + '"]:first');
                if ($a.length) {
                    $a.parent().addClass('selected');
                }
                <?php if (!$_smarty_tpl->tpl_vars['options']->value['is_ajax']){?>
                    waDesignLoad();
                <?php }?>
                themeFilesSearch();
            });

            function handleError(data, $dialog) {
                let error = '';
                if (typeof data.errors == 'string') {
                    error += (error ? '\n' : '') + data.errors;
                } else {
                    for (let error_item in data.errors) {
                        if(data.errors.hasOwnProperty(error_item)) {
                            error += (error ? '\n' : '') + data.errors[error_item][0];
                        }
                    }
                }
                if ($dialog.length) {
                    $dialog.find(".wa-theme-dialog-error").html(error + '<br><br>');
                } else if ($(".wa-theme-dialog-error:first:visible").length) {
                    $(".wa-theme-dialog-error:first:visible").html('<br><br>' + error + '<br><br>');
                } else {
                    alert('Error:' + error);
                }
                $dialog.find("[type=submit]").removeClass('disabled');
            }

            $(".js-theme-upload-link").on('click', function (e) {
                e.preventDefault();

                if (!waDesignConfirm()) return false;
                const $upload_dialog = $("#wa-theme-upload-dialog");

                $(".wa-theme-dialog-error").text('');

                $upload_dialog.find("div.loading").hide();
                let $wrapper = $upload_dialog.clone();
                $.waDialog({
                    $wrapper,
                    onOpen($dialog, dialog) {
                        let $form = $dialog.find('form:first'),
                            $input_file = $dialog.find("#wa-input-file"),
                            $submit_btn = $dialog.find('[type="submit"]');

                        $dialog.find(".upload-area").waUpload({
                            is_uploadbox: true
                        });

                        $form.on('submit', function (e) {
                            e.preventDefault();
                            $submit_btn.addClass('disabled')
                            $dialog.find("div.loading").show();
                            const formData = new FormData(this);

                            postData($(this).attr('action'), formData)
                                .then(
                                    (res) => {
                                        try {
                                            let response = $.parseJSON(res);
                                            if (response.status === 'fail') {
                                                $dialog.find("div.loading").hide();
                                                $input_file.val('');
                                                handleError(response, $dialog);
                                            } else if (response.status === 'ok') {
                                                dialog.close();
                                                location.reload();
                                            }
                                        }catch (e){
                                            let response = {
                                                'errors': []
                                            };
                                            let message = $(res).find('h1:first, h2:first');
                                            if (message.length) {
                                                response.errors.push([message.text()]);
                                            } else {
                                                response.errors.push(['JavaScript error: ' + e.message]);
                                            }
                                            $dialog.find("div.loading").hide();
                                            $input_file.val('');
                                            handleError(response, $dialog);
                                        }

                                    },
                                    (error) => {
                                        console.error(error)
                                    }
                                );
                        })
                    }
                });
            });

            $(function () {
                $(document).trigger('wa_design_inited');

                initFixSidebarBottomOffset();
            });

            function initFixSidebarBottomOffset () {
                const nav_height = $('#wa-nav').height() || 0;
                const offset_top = $('.sidebar-header').offset().top;
                document.documentElement.style.cssText = `--sidebar-scroll-offset:${ offset_top }px`;
                const fixSidebarScrollOffset = () => {
                    const new_offset_top = offset_top - Math.min(offset_top, document.documentElement.scrollTop);
                    document.documentElement.style.setProperty('--sidebar-scroll-offset', `${ Math.max(new_offset_top, nav_height) }px`);
                };
                let timer = null;

                $(document).off('scroll.theme_sidebar').on('scroll.theme_sidebar', function () {
                    if (timer) {
                        return;
                    }

                    fixSidebarScrollOffset();

                    timer = setTimeout(() => {
                        fixSidebarScrollOffset();
                        timer = null;
                    }, 200);
                });
            }


            function themeFilesSearch(){
                const self = this;

                // Обработчик для динамически загружаемого контента
                $(document).on('input', '.js-theme-files-search', function() {
                    const $searchInput = $(this);
                    const searchQuery = $searchInput.val().toLowerCase().trim();
                    const $fileList = $searchInput.closest('#wa-design-sidebar').find('.wa-theme-templates');

                    if (!searchQuery) {
                        // Если поиск пустой, показываем все элементы и сбрасываем раскрытие
                        $fileList.find('li').show();
                        $fileList.find('.js-directory-toggle').each(function() {
                            const $toggle = $(this);
                            const $submenu = $toggle.siblings('ul.menu');
                            if ($toggle.attr('data-expanded') === 'true') {
                                $submenu.slideUp();
                                $toggle.attr('data-expanded', 'false');
                            }
                        });
                        return;
                    }

                    // Функция для раскрытия всех родительских папок
                    const expandParents = function($element) {
                        let $parent = $element.parent().closest('li');
                        while ($parent.length) {
                            const $toggle = $parent.find('.js-directory-toggle').first();
                            if ($toggle.length) {
                                const $submenu = $toggle.siblings('ul.menu');
                                if ($toggle.attr('data-expanded') !== 'true') {
                                    $submenu.slideDown();
                                    $toggle.attr('data-expanded', 'true');
                                }
                            }
                            $parent = $parent.parent().closest('li');
                        }
                    };

                    // Функция для проверки, содержит ли папка совпадающие файлы
                    const folderContainsMatch = function($folder) {
                        let hasMatch = false;
                        const $submenu = $folder.siblings('ul.menu');
                        const $children = $submenu.find('li');

                        $children.each(function() {
                            const $child = $(this);
                            const $childToggle = $child.find('.js-directory-toggle');

                            if ($childToggle.length) {
                                // Это папка, проверяем рекурсивно
                                if (folderContainsMatch($childToggle)) {
                                    hasMatch = true;
                                }
                            } else {
                                // Это файл
                                const filename = $child.attr('data-filename') || '';
                                if (filename.toLowerCase().indexOf(searchQuery) !== -1) {
                                    hasMatch = true;
                                }
                            }
                        });

                        return hasMatch;
                    };

                    // Обрабатываем все элементы списка
                    $fileList.find('li').each(function() {
                        const $li = $(this);
                        const $toggle = $li.find('.js-directory-toggle');

                        if ($toggle.length) {
                            // Это папка
                            const folderName = $toggle.find('span').not('.toggle-icon').text().toLowerCase();
                            const folderMatches = folderName.indexOf(searchQuery) !== -1;
                            const containsMatch = folderContainsMatch($toggle);

                            if (folderMatches || containsMatch) {
                                $li.show();
                                // Раскрываем папку, если она содержит совпадения
                                if (containsMatch) {
                                    const $submenu = $toggle.siblings('ul.menu');
                                    if ($toggle.attr('data-expanded') !== 'true') {
                                        $submenu.slideDown();
                                        $toggle.attr('data-expanded', 'true');
                                    }
                                }
                            } else {
                                $li.hide();
                            }
                        } else {
                            // Это файл
                            const filename = $li.attr('data-filename') || '';
                            if (filename.toLowerCase().indexOf(searchQuery) !== -1) {
                                $li.show();
                                // Раскрываем все родительские папки
                                expandParents($li);
                            } else {
                                $li.hide();
                            }
                        }
                    });
                });
            }

            $('#wa-design-sidebar').on('click', '.js-directory-toggle', function(e) {
                e.preventDefault();
                const $menu = $(this).next('.menu');
                const is_visible = $menu.is(':visible');

                if(is_visible) {
                    $menu.slideUp();
                } else {
                    $menu.slideDown();
                }
            });
        </script>
            <?php if (!$_smarty_tpl->tpl_vars['wa']->value->isMobile()){?></div><?php }?>
</div>
<?php }} ?>