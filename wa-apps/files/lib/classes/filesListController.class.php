...
class filesListController extends filesController
{
    ...
    {
        if ($this->order === null) {
            $order = waRequest::get('order', [], waRequest::TYPE_ARRAY_TRIM);
            if (!empty($order[0]) && !in_array($order[0], $this->orders)) {
                unset($order);
            }
            if (!$order) {
            ...
            if (!empty($order[1]) && !in_array(strtolower($order[1]), ['asc', 'desc'])) {
                unset($order[1]);
            }
            if (empty($order[1]) && !empty($order[0])) {
                $order[1] = 'asc';
            }
