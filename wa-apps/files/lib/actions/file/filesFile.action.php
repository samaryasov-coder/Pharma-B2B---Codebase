...
class filesFileAction extends filesController
{
    ...
    public function getFile()
    {
        ...
        if ($file['file_type'] === filesConfig::FILE_TYPE_TEXT) {
            $maxlen = $this->getTextFileShowMaxSize();
            if (!empty($file['path'])) {
                $file['content'] = $this->getFileContent($file['path'], 0, $maxlen);
                $file['content_size'] = @filesize($file['path']);
            } elseif (!empty($file['source_id'])) {
                $file['content_size'] = (int) ifset($file['size'], 0);
                $file['content'] = $this->getRemoteFileContent($file, $maxlen);
            } else {
                $file['content'] = '';
                $file['content_size'] = 0;
            }
    ...
    private function getRemoteFileContent($file, $maxlen)
    {
        $source = filesSource::factory($file['source_id']);
        if (!$source) {
            return '';
        }
        $stream = $source->download($file, filesSource::DOWNLOAD_STREAM);
        if (!is_resource($stream)) {
            return '';
        }
        $content = stream_get_contents($stream, $maxlen);
        fclose($stream);
