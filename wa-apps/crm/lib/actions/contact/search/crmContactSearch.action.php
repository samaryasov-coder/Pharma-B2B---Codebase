...
class crmContactSearchAction extends crmBackendViewAction
{
            ...
            foreach ($conds as $section_id => $cond) {
                foreach ($cond as $item_id => $item_cond_ar) {
                    ...
                    foreach ($item_cond_ar as $item_cond) {
                        ...
                        $item_id_full = "{$section_id}.{$item_id}";
                        $it = crmContactsSearchHelper::getItem($item_id_full, $h, array(
                            'unwrap_values' => $this->getUnwrapValuesOptions($item_id_full),
                            'count' => false
                        ));
                        $items[] = array(
                            'item_id' => $it['id'],
    ...
    public function getItems($item_ids)
    {
        ...
        foreach ($item_ids as $item_id) {
            $item = crmContactsSearchHelper::getItem($item_id, null, array(
                'unwrap_values' => $this->getUnwrapValuesOptions($item_id),
                'count' => false
            ));
            if ($item) {
                $items[] = array(
    ...
    protected function getUnwrapValuesOptions($item_id)
    {
        if ($item_id === 'contact_info.creating') {
            return true;
        }
