 ...
 * @property $limit
 */
abstract class pb2bWaproModel extends waModel
{
                ...
                if (empty($table['right']) || !($table['right'] instanceof waModel)) {
                    ...
                    if ($table['query'] ?? false) {
                        if (empty($table['as'])) {
                            ...
                            if ($this->join_tables[$table['as']] instanceof pb2bWaproModel) {
    ...
    {
        $result = '';
        if ($value['sub'] instanceof pb2bWaproModel) {
            ...
            $this->log('При передаче в условие подзапроса, в ключе sub необходимо передать экземпляр pb2bWaproModel, ');//TODO doc link
        }
        return $result;
    }
    ...
    {
        if ($this->current_directive == 'group' || $check_select) {
            if (empty($this->select)) {
                $field = $col;
            } else {
    ...
    public final function showQuery(string $type = 'select'): void
    {
        ...
        waLog::log($types[$type], 'pb2bWaproModel.log');
    }
    ...
    {
        $escaped = $this->escapeField($field);
        return (int) $this->query(
            "SELECT EXISTS(SELECT 1 FROM {$this->table} WHERE $escaped = ?)",
            $value
        )->fetchField() === 1;
    }
    ...
    {
        if (class_exists('pb2bWaproHelper')) {
            pb2bWaproHelper::logRecord('pb2bWaproModel', array($record.pb2bWaproHelper::getAddressPerson().'!'));
        } else {
            waLog::log($record.' товарищ!', 'pb2bWaproModel.log');
        }
    }
