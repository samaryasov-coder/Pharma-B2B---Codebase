...
/**
 * Class pb2bWaproCollection
 * @property $hash
 * @property $pagination_params
 */
abstract class pb2bWaproCollection
{
    use pb2bWaproBaseTrait;
    /**
     * @var array
     */
    ...
    /**
     * pb2bWaproCollection constructor.
     * @param null $hash
     * @throws waException
     */
            ...
            if (mb_strpos($value, $simile)) {
                ...
                if (count($value) == 2 && strlen($value[1])) {
                    ...
                    if ($this->checkHashValue($val)) {
                        ...
                        pb2bWaproModel::setElement($result[$field], $value[0], $val);
                    }
                }
                break;
            }
    ...
    public function getCollection(array $params = array()): array
    {
        ...
        if(isset($params['select'])) {
            $this->model->setSelect($params['select']);
        }
        $this->addWhere();
...
//        $this->pagination_params['total'] = ceil($this->pagination_params['total_count']/$this->pagination_params['limit']);
    ...
    protected function setDataTableResult(array &$result, array $params): void
    {
        if (!empty($result['base_data'])) {
            $col_templates = array();
            $fields = empty($params['fields']) ? pb2bWaproHelper::getFields($this->class_name) : $params['fields'];
                ...
                foreach ($fields as $code => $field) {
                    if (isset($field['viewed'])) {
                        ...
                        if (empty($col_templates[$code])) {
                            ...
                            $current_row[] = htmlspecialchars($cell, ENT_QUOTES, 'UTF-8');
                        } else {
                            $row['params'] = $params;
                            ...
                            $current_row[] = array_key_exists($code, $row) ? $row[$code] : '';
                        }
                    }
                }
     ...
     * @throws waDbException
     */
    protected function includeMultiple(pb2bWaproModel $model, string $field, string $include, array &$items, array $ids): void
    {
     ...
     * @throws waDbException
     */
    protected function include(pb2bWaproModel $model, string $field, string $include, array &$items): void
    {
        $ids = array();
     ...
     * @return string
     */
    static public function getFilterHash($filters): string
    {
        $result = array();
        foreach ($filters as $code => $value) 
        {
            if (isset($value['from'])) {
                if (!empty($value['from']) && strlen($value['from']) > 0) {
                    $result[] = $code.'>='.$value['from'];
                }
                if (!empty($value['to']) && strlen($value['to']) > 0) {
                    $result[] = $code.'<='.$value['to'];
                }
            } elseif (isset($value['date_from']) || isset($value['date_to'])) {
                if (!empty($value['date_from']) && strlen($value['date_from']) > 0) {
                    $date = DateTime::createFromFormat('d.m.Y', $value['date_from']);
                    $result[] = $code.'>='.$date->format('Y-m-d');
                }
                if (!empty($value['date_to']) && strlen($value['date_to']) > 0) {
                    $date = DateTime::createFromFormat('d.m.Y', $value['date_to']);
                    $result[] = $code.'<='.$date->format('Y-m-d');
                }
            } else {
                if (is_array($value)) {
