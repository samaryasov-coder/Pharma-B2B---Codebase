 ...
 */
abstract class pb2bWaproObject implements arrayAccess
{
     ...
     * @var ?pb2bWaproObject
     */
    protected ?pb2bWaproObject $subject = null;
    ...
    {
        if ($this->model instanceof pb2bWaproModel && !empty($value)) {
            if (is_array($value)) {
    ...
    {
        $result = pb2bWaproHelper::validate($data, $this->class_name);
        if (!$result['error']) {
            if (empty($this->id)) {
                $result['new'] = true;
            ...
            if (isset($this->model->fields['update_datetime'])) {
    ...
    public final function save(array $data): array
    {
        ...
        if (!$result['error']) {
            ...
            if (isset($this->id) && !empty($this->data)) {
                if (pb2bWaproHelper::arrayChangeCheck($this->data, $data)) {
    ...
    protected function preDelete(array &$data = array()): array
    {
        ...
        if ($this->id) {
            $result = array('error' => false, 'data' => $data, 'item' => $this->data);
        }
        return $result;
    }
     ...
     * @throws waException
     */
    public final function delete(array $data = array()): array
    {
    ...
    {
        $cases = pb2bWaproHelper::getConfigOption('objects');
    ...
    /**
     * @param ?string $hash
     * @return ?pb2bWaproCollection|null
     */
    public function getCollection(string $hash = null): ?pb2bWaproCollection
    {
        $collection = null;
        $collection_name = $this->app_id.ucfirst($this->subject->class_name ?? '').ucfirst($this->class_name).'Collection';
        if (class_exists($collection_name)) {
            $collection_name = new $collection_name($hash);
            if ($collection_name instanceof pb2bWaproCollection) {
