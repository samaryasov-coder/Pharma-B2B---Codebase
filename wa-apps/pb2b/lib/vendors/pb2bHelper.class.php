...
class pb2bHelper
{
     ...
     * @throws waException
     */
    static public function getConfigOption(array|string $name = null, string $field = null): ?array
    {
        $result = null;
        if (empty($name)) {
    ...
    /**
     * @param $model
     * @return pb2bModel|null
     * @throws waException
     */
    static public function getModel($model): ?pb2bModel
    {
        $result = null;
        $model = waSystem::getInstance()->getAppInfo()['id'].ucfirst($model).'Model';
        if (class_exists($model)) {
            $model = new $model();
            if ($model instanceof pb2bModel) {
                $result = $model;
            }
        }
        return $result;
    ...
    static public function dbPhpGenerate($app = null): void
    {
        ...
        foreach (pb2bModel::getTables($app) as $t) {
            $t = array_values($t)[0];
            if (!empty($t)) {
                $command .= ' '.$t;
            }
        ...
        if (!empty($command)) {
            ...
            waLog::dump("php wa.php generateDb ".$command, 'pb2bModel.log');
//            `php wa.php generateDb $command`;
        }
    }


    static public function pick(array $source, array $keys): array
    {
        return array_intersect_key($source, array_flip($keys));
    }

    static public function getCompanyTypeName(int|string $type)
    {
        static $types = null;
        if ($types === null) {
            $types = pb2bWaproHelper::getConfigOption('type_organization');
        }

        return $types[$type]['name'] ?? 'ИП';
    }

    static public function getStatusType(int|string $type)
    {
        static $types = null;

        if ($types === null) {
            $types = pb2bWaproHelper::getConfigOption('docflow_request_statuses');;
        }

        return $types[$type] ?? null;
    }

    static public function getFileSizeText($filepath){
        $units = ['B', 'KB', 'MB', 'GB', 'TB'];
        ...
        return sprintf("%.2f %s", $filesize / pow(1024, $factor), $units[$factor]);
    }
}