...
class pb2bRecoveryController extends pb2bFrontendController
{
    ...
    public function executeAction()
    {
        ...
        if ($contact == false)
            return $this->setErrorResponse($message)->withDetail($login_field, 'Данные не найдены');
        ...
        if (empty($code_id))
            return $this->setErrorResponse($message)->withDetail($login_field, $serviceCode->getLastError());
